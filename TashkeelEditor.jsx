...
export default function TashkeelEditor() {
  ...
  // Simulate auto-diacritization
  const handleAutoDiacritize = useCallback(async () => {
    setIsProcessing(true);
    // In production, this calls POST /api/v1/diacritize
    await new Promise((r) => setTimeout(r, 1500));
    // Mock: add fatha to all bare Arabic letters
    setSegments((prev) =>
      prev.map((s) => {
        if (ARABIC_RANGE.test(s.base) && s.marks.length === 0 && s.base.trim()) {
          return { ...s, marks: [DIACRITICS.fatha.char] };
        }
        return s;
      })
    );
